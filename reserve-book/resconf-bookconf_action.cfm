...
<p>#getUser.UserName# has requested to confirm the booking for #getBooking.VesselName# from #LSDateFormat(getBooking.StartDate, request.datemask)# to #LSDateFormat(getBooking.EndDate, request.datemask)#. This is for <CFIF #URL.jetty#>#northorsouth# Jetty<cfelse>the Drydock</CFIF>.</p>
	</cfmail>
...
<!--- create structure for sending to mothership/success page. --->
	...
	<cfset Session.Eng.Success.Message = "<div align='left'>Your confirmation request for the booking for <strong>#getBooking.vesselName#</strong> from #LSDateFormat(CreateODBCDate(getBooking.startDate), request.datemask)# to #LSDateFormat(CreateODBCDate(getBooking.endDate), request.datemask)# is now pending.  EGD administration has been notified of your request.  You will receive a follow-up email responding to your request shortly.</div>">
  ...
  <cfset Session.Eng.Success.Link = "#returnTo#?#urltoken#&CID=#url.CID##variables.dateValue#">
	...
	<cfset Session.Fra.Success.Message = "<div align='left'>Votre demande de confirmation de la r&eacute;servation pour le <strong>#getBooking.vesselName#</strong> du #LSDateFormat(CreateODBCDate(getBooking.startDate), request.datemask)# au #LSDateFormat(CreateODBCDate(getBooking.endDate), request.datemask)#  est en cours de traitement. L'administration de la CSE a &eacute;t&eacute; avis&eacute;e de votre demande. Vous recevrez sous peu un courriel de suivi en r&eacute;ponse &agrave; votre demande.</div>">
