...
<cfoutput><SCRIPT language="javascript" type="text/javascript" src="#RootDir#scripts/Tokenizer.js"></SCRIPT></cfoutput>
...
function getCalendar(formName, fieldName) {
	...
	if (flag) {
		miniCal = window.open("<CFOUTPUT>#RootDir#</cfoutput>text/utils/calendar.cfm?lang=<cfoutput>#lang#</cfoutput>&formName=" + formName + "&year=" + yearValue + "&month=" + monthValue + "&fieldName=" + fieldName<CFIF IsDefined('Variables.BookingLen')> + "&len=" + <CFOUTPUT>#Variables.BookingLen#</CFOUTPUT></CFIF>, "CalendarWindow", "left=30,top=30,width=450,height=250,resizable=yes,scrollbars=no");
